...
SPATIAL_EPS_METERS = 255.06      # max deadhead distance between consecutive trips (meters)
TEMPORAL_EPS_MIN   = 26.97       # max deadhead time gap between consecutive trips (minutes)

# DBSCAN parameters
DBSCAN_EPS = 0.695                # we normalize so "feasible neighbor" tends to be <= 1
MIN_TRIPS_PER_CLUSTER = 4     # 2 => each core trip needs at least 1 neighbor (itself counts)
...
START_TIME_WINDOW_MIN = 240.0   # Classify day per XX minutes
...
def spatio_temporal_distance_time_only(a, b):
    """Metric for *subclustering* only: ignore space, use time only."""
    return spatio_temporal_distance_with_weights(a, b, space_w=0.0, time_w=1.0)
...
def cluster_trips(trips, max_size=MAX_TRIPS_PER_CLUSTER, max_depth=12):
    """
    Run DBSCAN using the chain-feasible metric.

    If any cluster is larger than `max_size`, recursively re-cluster *within that
    cluster* and add the resulting sub-clusters to the global set.

    This repeats until all clusters are <= max_size, or until we stop making
    progress (then we fall back to a deterministic time-split).
    """
    # Initial global pass (uses the configured SPACE_WEIGHT/TIME_WEIGHT)
    initial_parts = _dbscan_partition(trips, metric_func=spatio_temporal_distance, verbose=True)

    # Work queue of (cluster_trips_list, depth)
    queue = [(p, 0) for p in initial_parts]

    global_clusters = {}
    cid = 0

    while queue:
        c_trips, depth = queue.pop(0)

        if len(c_trips) <= max_size:
            global_clusters[cid] = c_trips
            cid += 1
            continue

        # Hard stop: avoid pathological infinite recursion
        if depth >= max_depth:
            for chunk in split_oversized_cluster_by_time(c_trips, max_size):
                global_clusters[cid] = chunk
                cid += 1
            continue

        # Re-cluster inside this oversized cluster (TIME-ONLY metric)
        sub_parts = _dbscan_partition(c_trips, metric_func=spatio_temporal_distance_time_only, verbose=False)

        # If DBSCAN can't split it (or doesn't change anything), fall back to time split
        no_progress = (
            len(sub_parts) == 1 and
            len(sub_parts[0]) == len(c_trips)
        )
        ...
        # Otherwise, add sub-clusters back to the queue for size checking
        for sp in sub_parts:
