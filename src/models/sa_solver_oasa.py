...
def solve_md_vsp_tw_sa_from_instance(
    ...
    IMPORTANT:
    - The GTFS instance loader, constants, and node naming are kept consistent with the MIP file.
    ...
    if time_limit_sec is None:
        time_limit_sec = globals().get("SA_TIME_LIMIT_SEC", None)

    if gurobi_initial_time_limit is None:
        gurobi_initial_time_limit = globals().get("GUROBI_TIME_LIMIT_SEC", None)
    ...
    # Objective weights: same structure as the MIP objective.
    ...
    def leg_metrics(vehicle_id, i_node, j_node):
        ...
        elapsed = dist_m / AVG_SPEED_M_PER_MIN
        energy = dist_m * THETA_FACTOR
        cost = instance.meta["travel_cost"] * elapsed

        return dist_m, elapsed, energy, cost
    ...
    def build_gurobi_initial_state():
        ...
        """
        try:
            _, mip_schedules, _ = solve_md_vsp_tw_from_instance(
                instance,
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if time_limit_sec is not None:
            elapsed = time.perf_counter() - solve_start_time
            if elapsed >= float(time_limit_sec):
                time_limit_reached = True
                print(f"SA time limit reached after {elapsed:.2f} seconds.")
                break
    ...
    print(f"Time limit reached:      {time_limit_reached}")
    ...
    report_lines.append(f"Iterations: {iterations_completed:,}")
    report_lines.append(f"Time limit reached: {time_limit_reached}")
    ...
    if solution_data:
        try:
            ...
            # Allows the solver to be imported and called without running __main__.
            pass
...
def solve_md_vsp_tw_from_instance(
    ...
    - mip_gap controls Gurobi's relative MIP optimality gap, e.g. 0.50 for 50%.
    """
    ...
    if has_incumbent:
        ...
        if model.status == GRB.OPTIMAL:
            report_lines.append(f"Gurobi solution found with total cost: {model.ObjVal:.2f}")
        else:
            report_lines.append(
                f"Gurobi incumbent found with total cost: {model.ObjVal:.2f} "
                f"and gap {model.MIPGap*100:.2f}%"
            )
        ...
        if solution_data and write_solution_json:
            try:
                ...
                # Allows the solver to be imported and used before __main__ creates output_dir.
                pass
    else:
        report_lines.append("No Gurobi incumbent was found or the model was infeasible.")
...
if __name__ == "__main__":
    ...
    # Standalone SA controls.
    SA_TIME_LIMIT_SEC = 18000
    ...
    output_dir = os.path.join(project_root, "..", "output", f"SA_EB_TW_run_{timestamp}")
    ...
    # 4. Solve instance
    print("--- Part 4: Solving the Instance with Simulated Annealing ---")
    solution_report, schedules, variable_report_str = solve_md_vsp_tw_sa_from_instance(
        instance,
        use_gurobi_initial_solution=True,
        gurobi_initial_gap=0.4,
        gurobi_initial_time_limit=GUROBI_TIME_LIMIT_SEC,
        gurobi_initial_output_flag=1,
        strict_feasibility=True,
        time_limit_sec=SA_TIME_LIMIT_SEC,
    )
