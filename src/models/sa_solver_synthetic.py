...
MIN_POSITIVE_DISTANCE = 1e-3

def positive_distance(value: float) -> float:
    return max(float(value), MIN_POSITIVE_DISTANCE)
...
def load_instance_from_txt(path: str):
    ...
    for row in trip_rows:
        ...
        time_distance = positive_distance(dist_val)
        ...
        trip.trip_length = time_distance
...
def _copy_sa_state(state):
    """
    Lightweight copy for the SA state.

    State representation:
    - state[vehicle_id] = ordered list of internal node ids assigned to that vehicle.
    - Internal nodes may be trip nodes (T#) and charging-station nodes (C#).
    - When the initial state comes from Gurobi, the route is copied as-is
      after removing only the origin/destination depot labels.
    """
    return {k: list(v) for k, v in state.items()}


def _build_sa_network_data(instance: ProblemInstance):
    """
    Build the same vehicle/node dictionaries used by the MIP model.
    This keeps the SA implementation aligned with the original data structures.
    """
    ...
    return {
        ...
        "trips_map": trips_map,
        "trip_ids": trip_ids,
        "charging_station_map": charging_station_map,
        "charging_station_ids": charging_station_ids,
        "charging_station_set": charging_station_set,
        "origin_nodes": origin_nodes,
        "dest_nodes": dest_nodes,
        "min_energy_to_cs": min_energy_to_cs,
    }


def solve_md_vsp_tw_sa_from_instance(
    instance: ProblemInstance,
    ...
    random_seed: int = 43,
    use_gurobi_initial_solution: bool = True,
    gurobi_initial_gap: float = 0.50,
    gurobi_initial_time_limit: float = None,
    gurobi_initial_output_flag: int = 1,
    output_dir: str = None,
    bus_end_states_dir: str = None,
    run_timestamp: str = None,
    mild_cross_vehicle_probability: float = 0.20,
    local_shift_radius: int = 5,
    strict_feasibility: bool = True,
    penalty_tolerance: float = 1e-6
):
    """
    Solves the MD-VSP-TW with an Energy Buffer extension using Simulated Annealing.

    IMPORTANT:
    - The TXT instance loader, constants, and node naming are kept consistent with the MIP file.
    - The preferred initial state is the Gurobi incumbent route copied as-is:
      origin/destination depot nodes are removed, but trip nodes and charging-station nodes are preserved.
    - SA neighborhood moves are still mild, but the default radius, cross-vehicle probability,
      temperature and cooling are tuned to explore more around the Gurobi incumbent.
    - The best state is initialized with the Gurobi-seeded state, so the final reported SA state
      cannot be worse than the starting state under the SA evaluator.
    - If strict_feasibility is True, candidates with a positive feasibility penalty are rejected
      before applying the SA acceptance rule.
    """

    random.seed(random_seed)

    data = _build_sa_network_data(instance)

    vehicles = data["vehicles"]
    vehicle_to_depot = data["vehicle_to_depot"]
    trips_map = data["trips_map"]
    trip_ids = data["trip_ids"]
    charging_station_map = data["charging_station_map"]
    charging_station_ids = data["charging_station_ids"]
    charging_station_set = data["charging_station_set"]
    origin_nodes = data["origin_nodes"]
    dest_nodes = data["dest_nodes"]
    min_energy_to_cs = data["min_energy_to_cs"]

    # Objective weights: same structure as the MIP objective used in this synthetic-data file.
    waiting_cost_lambda = instance.meta["lambda"]
    lambda_1 = waiting_cost_lambda
    lambda_2 = 0
    lambda_3 = waiting_cost_lambda

    # Penalties are deliberately larger than normal operating costs.
    penalty_cover = 1000000.0
    penalty_arc = 100000.0
    penalty_time = 100000.0
    penalty_energy = 100000.0
    penalty_charger = 100000.0

    def route_has_trip(route_nodes):
        return any(node_id in trips_map for node_id in route_nodes)

    def service_time(node_id, vehicle_id, tau_values):
        if node_id in trips_map:
            return trips_map[node_id].trip_length
        if node_id in charging_station_set:
            return tau_values.get((vehicle_id, node_id), 0.0)
        return 0.0

    def point_from_node(vehicle_id, node_id):
        if node_id in trips_map:
            return trips_map[node_id].end_point
        if node_id in charging_station_set:
            return charging_station_map[node_id].location
        return vehicle_to_depot[vehicle_id].location

    def point_to_node(vehicle_id, node_id):
        if node_id in trips_map:
            return trips_map[node_id].start_point
        if node_id in charging_station_set:
            return charging_station_map[node_id].location
        return vehicle_to_depot[vehicle_id].location

    def leg_metrics(vehicle_id, i_node, j_node):
        from_point = point_from_node(vehicle_id, i_node)
        to_point = point_to_node(vehicle_id, j_node)

        dist_val = distance.euclidean(
            [from_point.y, from_point.x],
            [to_point.y, to_point.x]
        )

        elapsed = positive_distance(dist_val)
        energy = dist_val * THETA_FACTOR
        cost = instance.meta["travel_cost"] * elapsed

        return dist_val, elapsed, energy, cost

    def is_allowed_arc(vehicle_id, i_node, j_node):
        """
        Topological/valid-inequality check matching the original arc generation.
        Time and energy feasibility are still evaluated separately with penalties.
        """
        if i_node == origin_nodes[vehicle_id] and j_node in trips_map:
            _, travel_time, _, _ = leg_metrics(vehicle_id, i_node, j_node)
            return travel_time <= trips_map[j_node].start_time_window[1]

        if i_node in trips_map and j_node in trips_map:
            _, travel_time, _, _ = leg_metrics(vehicle_id, i_node, j_node)
            l_i, _ = trips_map[i_node].start_time_window
            _, u_j = trips_map[j_node].start_time_window
            return l_i + trips_map[i_node].trip_length + travel_time <= u_j

        if i_node in trips_map and j_node == dest_nodes[vehicle_id]:
            return True

        if i_node in trips_map and j_node in charging_station_set:
            return True

        if i_node in charging_station_set and j_node in trips_map:
            return True

        return False

    def evaluate_state(state, detailed: bool = False):
        """
        Evaluates a complete route state.

        Charging stations are not inserted greedily here. The route is traversed
        exactly as represented in the SA state. Timing is then shifted backwards
        within the allowed time windows so that avoidable waiting is absorbed at
        the depot or inside trip time-window slack, matching the MIP timing logic
        more closely than a purely earliest-start simulation.
        """
        ...
        # Coverage check. Only trip nodes count for service coverage.
        assigned_trips = []
        invalid_count = 0
        for vehicle in vehicles:
            for node_id in state.get(vehicle, []):
                if node_id in trips_map:
                    assigned_trips.append(node_id)
                elif node_id not in charging_station_set:
                    invalid_count += 1

        assigned_set = set(assigned_trips)
        missing = set(trip_ids) - assigned_set
        duplicate_count = len(assigned_trips) - len(assigned_set)

        if missing:
            penalty += penalty_cover * len(missing)
        if duplicate_count > 0:
            penalty += penalty_cover * duplicate_count
        if invalid_count > 0:
            penalty += penalty_cover * invalid_count

        def node_time_window(vehicle_id, node_id):
            if node_id in trips_map:
                return trips_map[node_id].start_time_window
            if node_id in charging_station_set:
                return (0.0, BIG_M)
            if node_id.startswith("O") or node_id.startswith("D"):
                vehicle_index = vehicles.index(vehicle_id)
                return (instance.meta["buses_availability_times"][vehicle_index], BIG_M)
            return (0.0, BIG_M)

        for vehicle in vehicles:
            internal_route_nodes = [
                node_id for node_id in state.get(vehicle, [])
                if (node_id in trips_map) or (node_id in charging_station_set)
            ]
            o_node = origin_nodes[vehicle]
            d_node = dest_nodes[vehicle]
            vehicle_index = vehicles.index(vehicle)

            if not route_has_trip(internal_route_nodes):
                # Keep unused vehicles explicitly available at their depot.
                start_time = instance.meta["buses_availability_times"][vehicle_index]
                start_energy = instance.meta["buses_initial_soc"][vehicle_index]
                schedules[vehicle] = [o_node]
                node_times[(vehicle, o_node)] = start_time
                e_bar_values[(vehicle, o_node)] = start_energy
                visit_details[vehicle] = [{
                    "node": o_node,
                    "time": start_time,
                    "E_pre": start_energy,
                    "G": 0.0,
            ...
            operational_cost += lambda_2

            full_route_nodes = [o_node] + internal_route_nodes + [d_node]
            schedules[vehicle] = list(full_route_nodes)
            visit_details[vehicle] = []

            # ----------------------------------------------------------
            # 1) Validate arcs, compute travel costs and energy profile.
            # ----------------------------------------------------------
            leg_travel_times = []
            leg_energy_consumption = []
            leg_costs = []

            for idx in range(len(full_route_nodes) - 1):
                i_node = full_route_nodes[idx]
                j_node = full_route_nodes[idx + 1]

                if not is_allowed_arc(vehicle, i_node, j_node):
                    penalty += penalty_arc

                _, travel_time, move_energy, move_cost = leg_metrics(vehicle, i_node, j_node)
                leg_travel_times.append(travel_time)
                leg_energy_consumption.append(move_energy)
                leg_costs.append(move_cost)

                active_arcs[(vehicle, i_node, j_node)] = 1.0
                operational_cost += move_cost

            service_durations = [0.0 for _ in full_route_nodes]
            current_energy = instance.meta["buses_initial_soc"][vehicle_index]

            e_bar_values[(vehicle, o_node)] = current_energy
            e_pre_values[(vehicle, o_node)] = current_energy
            g_values[(vehicle, o_node)] = 0.0

            for idx in range(1, len(full_route_nodes)):
                prev_node = full_route_nodes[idx - 1]
                node_id = full_route_nodes[idx]
                e_pre = current_energy - leg_energy_consumption[idx - 1]

                if node_id in trips_map:
                    g_value = trips_map[node_id].eta
                    e_bar = e_pre - g_value
                    service_durations[idx] = trips_map[node_id].trip_length

                    pre_violation = max(0.0, PHI_MIN - e_pre)
                    post_violation = max(0.0, PHI_MIN - e_bar)
                    reach_violation = max(
                        0.0,
                        PHI_MIN + min_energy_to_cs[node_id] - e_bar
                    )
                    energy_violation = pre_violation + post_violation + reach_violation
                    if energy_violation > 0.0:
                        penalty += penalty_energy * (1.0 + energy_violation)

                elif node_id in charging_station_set:
                    g_value = e_pre - PHI_MAX
                    e_bar = PHI_MAX
                    charge_time = max(0.0, (e_bar - e_pre) / CHARGING_RATE_KWH_PER_MINUTE)
                    service_durations[idx] = charge_time
                    tau_values[(vehicle, node_id)] = charge_time

                    if e_pre < PHI_MIN:
                        penalty += penalty_energy * (1.0 + (PHI_MIN - e_pre))

                else:
                    # Destination depot.
                    g_value = 0.0
                    e_bar = e_pre
                    service_durations[idx] = 0.0

                    if e_pre < PHI_MIN:
                        penalty += penalty_energy * (1.0 + (PHI_MIN - e_pre))

                e_pre_values[(vehicle, node_id)] = e_pre
                g_values[(vehicle, node_id)] = g_value
                e_bar_values[(vehicle, node_id)] = e_bar
                current_energy = e_bar

            # ----------------------------------------------------------
            # 2) Optimize timing for this fixed route.
            #
            # Earliest-start timing overestimates the MIP objective because
            # the MIP can delay depot departure and trip starts inside their
            # time windows. The backward pass shifts service starts as late as
            # possible without delaying the already feasible successor.
            # ----------------------------------------------------------
            route_times = [0.0 for _ in full_route_nodes]
            lower_bounds = []
            upper_bounds = []

            for node_id in full_route_nodes:
                lb, ub = node_time_window(vehicle, node_id)
                lower_bounds.append(lb)
                upper_bounds.append(ub)

            route_times[0] = lower_bounds[0]

            for idx in range(1, len(full_route_nodes)):
                earliest_arrival = (
                    route_times[idx - 1]
                    + service_durations[idx - 1]
                    + leg_travel_times[idx - 1]
                )
                route_times[idx] = max(lower_bounds[idx], earliest_arrival)

                if full_route_nodes[idx] in trips_map:
                    time_violation = max(0.0, route_times[idx] - upper_bounds[idx])
                    if time_violation > 0.0:
                        penalty += penalty_time * (1.0 + time_violation)

            # Backward shift: absorb waiting into previous node slack, especially
            # depot departure time, whose waiting is not penalized by the MIP.
            for idx in range(len(full_route_nodes) - 2, -1, -1):
                latest_without_wait = (
                    route_times[idx + 1]
                    - service_durations[idx]
                    - leg_travel_times[idx]
                )
                shifted_time = min(upper_bounds[idx], latest_without_wait)
                route_times[idx] = max(lower_bounds[idx], shifted_time)
            ...
            for idx, node_id in enumerate(full_route_nodes):
                node_times[(vehicle, node_id)] = route_times[idx]

            for idx in range(len(full_route_nodes) - 1):
                i_node = full_route_nodes[idx]
                j_node = full_route_nodes[idx + 1]
                arrival_without_wait = (
                    route_times[idx]
                    + service_durations[idx]
                    + leg_travel_times[idx]
                )
                wait_time = max(0.0, route_times[idx + 1] - arrival_without_wait)

                if (j_node in trips_map) or (j_node in charging_station_set):
                    operational_cost += lambda_1 * wait_time
                    if wait_time > 1e-6:
                        waiting_values[(vehicle, i_node, j_node)] = wait_time

            for idx, node_id in enumerate(full_route_nodes):
                e_pre = e_pre_values.get((vehicle, node_id), e_bar_values.get((vehicle, node_id), 0.0))
                g_val = g_values.get((vehicle, node_id), 0.0)
                e_bar = e_bar_values.get((vehicle, node_id), e_pre)

                visit_details[vehicle].append({
                    "node": node_id,
                    "time": route_times[idx],
                    "E_pre": e_pre,
                    "G": g_val,
                    "E_bar": e_bar
                })
                ...
                if node_id in charging_station_set:
                    charge_time = tau_values.get((vehicle, node_id), 0.0)
                    ct_values[(vehicle, node_id)] = route_times[idx] + charge_time
                    charging_intervals.append({
                        "station": node_id,
                        "vehicle": vehicle,
                        "start": route_times[idx],
                        "end": route_times[idx] + charge_time
                    })

        # Charging station capacity approximation: penalize overlaps at the same station.
        intervals_by_station = {}
        for interval in charging_intervals:
            intervals_by_station.setdefault(interval["station"], []).append(interval)

        for cs_id, intervals in intervals_by_station.items():
            intervals.sort(key=lambda item: item["start"])
            for prev, nxt in zip(intervals, intervals[1:]):
                overlap = prev["end"] - nxt["start"]
                if overlap > 0.0:
                    penalty += penalty_charger * (1.0 + overlap)

        # Penalize using the same charging station more than once by the same vehicle.
        for vehicle in vehicles:
            vehicle_cs_visits = [
                item["station"] for item in charging_intervals
                if item["vehicle"] == vehicle
            ]
            repeated = len(vehicle_cs_visits) - len(set(vehicle_cs_visits))
            if repeated > 0:
                penalty += penalty_charger * repeated

        total_cost = operational_cost + penalty
    ...
    def build_greedy_initial_state(base_state=None, trips_to_insert=None):
        """
        Greedy insertion initializer/fallback.
        Trips are considered chronologically and inserted at the best current position.
        Charging stations already present in base_state are preserved.
        """
        if base_state is None:
            state = {vehicle: [] for vehicle in vehicles}
        else:
            state = _copy_sa_state(base_state)

        if trips_to_insert is None:
            sorted_trip_ids = sorted(trip_ids, key=lambda t: trips_map[t].start_time)
        else:
            sorted_trip_ids = sorted(trips_to_insert, key=lambda t: trips_map[t].start_time)
        ...
        for trip_id in sorted_trip_ids:
            ...
            for vehicle in vehicles:
                for pos in range(len(state[vehicle]) + 1):
                    candidate = _copy_sa_state(state)
                    candidate[vehicle].insert(pos, trip_id)
                    candidate_score = evaluate_state(candidate)["total_cost"]
    ...
    def build_gurobi_initial_state():
        """
        Build the SA initial solution from a Gurobi incumbent.

        The exact MIP is solved with a user-defined MIP gap, e.g. 0.50 for 50%.
        The resulting vehicle routes are converted back to the SA state format by
        removing only origin/destination depot nodes. Trip nodes and charging-station
        nodes are retained exactly in the order returned by Gurobi.
        """
        try:
            mip_result = solve_md_vsp_tw_from_instance(
                instance,
                waiting_cost_lambda=waiting_cost_lambda,
                output_dir=output_dir,
                bus_end_states_dir=bus_end_states_dir,
                run_timestamp=run_timestamp,
                mip_gap=gurobi_initial_gap,
                time_limit=gurobi_initial_time_limit,
                model_output_flag=gurobi_initial_output_flag
            )
            mip_schedules = mip_result[1]
        except gp.GurobiError as exc:
            print(f"Gurobi initializer failed: {exc}. Falling back to greedy insertion.")
            return None

        if not mip_schedules:
            print("Gurobi initializer did not return an incumbent. Falling back to greedy insertion.")
            return None
        ...
        for vehicle in vehicles:
            route = mip_schedules.get(vehicle, [])
            for node_id in route:
                # Keep the Gurobi route as-is, except for O#/D# depot labels.
                if node_id in charging_station_set:
                    state[vehicle].append(node_id)
                    continue

                if node_id not in trips_map:
                    continue

                if node_id in seen_trips:
                    duplicate_trips.append(node_id)
                    continue

                state[vehicle].append(node_id)
                seen_trips.add(node_id)

        missing_trips = [trip_id for trip_id in trip_ids if trip_id not in seen_trips]

        # This should normally be empty because the MIP has CoverTrip constraints.
        # The repair keeps the SA state complete if Gurobi stopped without a clean incumbent.
        if missing_trips:
            print(
                f"Gurobi initializer missed {len(missing_trips)} trips. "
                "Completing the state with greedy insertion."
            )
            state = build_greedy_initial_state(state, missing_trips)

        if duplicate_trips:
            print(
                f"Gurobi initializer returned {len(duplicate_trips)} duplicate trip entries. "
                "Duplicates were removed before SA."
            )
    ...
    def build_initial_state():
        """
        Build the SA starting point. Prefer the loose-gap Gurobi incumbent and
        keep greedy insertion as a fallback.
        """
        if use_gurobi_initial_solution:
            print(
                "Building initial SA solution with Gurobi "
                f"(MIPGap={gurobi_initial_gap:.2f})."
            )
            gurobi_state = build_gurobi_initial_state()
            if gurobi_state is not None:
                return gurobi_state, f"Gurobi incumbent copied as-is, MIPGap={gurobi_initial_gap:.2f}"

        print("Building initial SA solution with greedy insertion.")
        return build_greedy_initial_state(), "Greedy insertion"

    def trip_positions(state):
        positions = []
        for vehicle in vehicles:
            for pos, node_id in enumerate(state.get(vehicle, [])):
                if node_id in trips_map:
                    positions.append((vehicle, pos, node_id))
        return positions

    def charging_positions(state):
        positions = []
        for vehicle in vehicles:
            for pos, node_id in enumerate(state.get(vehicle, [])):
                if node_id in charging_station_set:
    ...
    def insert_trip_locally(candidate, from_vehicle, old_pos, trip_id):
        """
        Reinsert a removed trip with a small perturbation.
        Most moves stay inside the same vehicle route; a small share moves to another vehicle.
        """
        if random.random() > mild_cross_vehicle_probability:
            to_vehicle = from_vehicle
            lower = max(0, old_pos - local_shift_radius)
            upper = min(len(candidate[to_vehicle]), old_pos + local_shift_radius)
            insert_pos = random.randint(lower, upper)
        else:
            to_vehicle = random.choice(vehicles)
            insert_pos = random.randrange(len(candidate[to_vehicle]) + 1)

        candidate[to_vehicle].insert(insert_pos, trip_id)

    def random_neighbor(state):
        """
        Generate a mild neighbor around the incumbent.

        The route structure is perturbed locally instead of being rebuilt. This keeps
        the SA search close to the Gurobi incumbent unless an accepted move improves it.
        """
        candidate = _copy_sa_state(state)
        trip_pos = trip_positions(candidate)
        cs_pos = charging_positions(candidate)

        if not trip_pos:
            return candidate

        operations = ["local_shift", "adjacent_swap", "cross_vehicle_swap"]
        weights = [0.45, 0.20, 0.22]

        if cs_pos and len(charging_station_ids) > 1:
            operations.append("change_charger")
            weights.append(0.05)

        if cs_pos:
            operations.append("move_charger_one_step")
            weights.append(0.04)
            operations.append("remove_charger")
            weights.append(0.02)

        if charging_station_ids:
            operations.append("insert_charger")
            weights.append(0.02)

        operation = random.choices(operations, weights=weights, k=1)[0]

        if operation == "local_shift":
            vehicle, pos, trip_id = random.choice(trip_pos)
            candidate[vehicle].pop(pos)
            insert_trip_locally(candidate, vehicle, pos, trip_id)

        elif operation == "adjacent_swap":
            swap_candidates = []
            for vehicle in vehicles:
                route = candidate[vehicle]
                trip_indexes = [idx for idx, node_id in enumerate(route) if node_id in trips_map]
                for idx in range(len(trip_indexes) - 1):
                    swap_candidates.append((vehicle, trip_indexes[idx], trip_indexes[idx + 1]))

            if swap_candidates:
                vehicle, p1, p2 = random.choice(swap_candidates)
                candidate[vehicle][p1], candidate[vehicle][p2] = candidate[vehicle][p2], candidate[vehicle][p1]

        elif operation == "cross_vehicle_swap":
            if len(trip_pos) >= 2:
                first, second = random.sample(trip_pos, 2)
                v1, p1, _ = first
                v2, p2, _ = second
                candidate[v1][p1], candidate[v2][p2] = candidate[v2][p2], candidate[v1][p1]

        elif operation == "change_charger":
            vehicle, pos, old_cs = random.choice(cs_pos)
            alternatives = [cs_id for cs_id in charging_station_ids if cs_id != old_cs]
            if alternatives:
                candidate[vehicle][pos] = random.choice(alternatives)

        elif operation == "move_charger_one_step":
            vehicle, pos, _ = random.choice(cs_pos)
            route = candidate[vehicle]
            directions = []
            if pos > 1:
                directions.append(-1)
            ...
            if directions:
                ...
                route[pos], route[new_pos] = route[new_pos], route[pos]

        elif operation == "remove_charger":
            vehicle, pos, _ = random.choice(cs_pos)
            candidate[vehicle].pop(pos)

        elif operation == "insert_charger":
            # Insert after an existing trip only when another node follows, avoiding a C -> D terminal arc.
            insertion_candidates = [
                (vehicle, pos)
                for vehicle, pos, _ in trip_pos
                if pos < len(candidate[vehicle]) - 1
            ]
            if insertion_candidates:
                vehicle, pos = random.choice(insertion_candidates)
                insert_pos = pos + 1
                candidate[vehicle].insert(insert_pos, random.choice(charging_station_ids))
    ...
    initial_state, initial_solution_source = build_initial_state()
    current_state = _copy_sa_state(initial_state)
    current_eval = evaluate_state(current_state)
    initial_eval = dict(current_eval)

    if strict_feasibility and current_eval["penalty"] > penalty_tolerance:
        raise ValueError(
            "The initial SA state has a non-zero feasibility penalty. "
            "Strict feasibility requires a feasible starting solution. "
            f"Initial penalty = {current_eval['penalty']:.6f}"
        )

    if initial_temperature is None:
        # More exploratory than the previous mild setting, while still anchored to the Gurobi seed.
        initial_temperature = max(50.0, abs(current_eval["total_cost"]) * 0.03)

    best_state = _copy_sa_state(current_state)
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # Hard-feasibility mode: infeasible candidates are not considered by the
        # SA acceptance probability. This keeps the Markov chain inside the
        # feasible region detected by the SA evaluator.
        if strict_feasibility and candidate_eval["penalty"] > penalty_tolerance:
            rejected_infeasible_moves += 1
            temperature = max(min_temperature, temperature * cooling_rate)
            continue

        delta = candidate_eval["total_cost"] - current_eval["total_cost"]
        accept = False
        ...
        if delta <= 0.0:
            ...
            accept_probability = math.exp(-delta / temperature) if delta / temperature < 700 else 0.0
    ...
    detailed_eval = evaluate_state(best_state, detailed=True)

    print("\n===== Simulated Annealing Performance Summary =====")
    print(f"Iterations:              {max_iterations:,}")
    print(f"Accepted Moves:          {accepted_moves:,}")
    print(f"Improving Moves:         {improved_moves:,}")
    print(f"Rejected Infeasible:     {rejected_infeasible_moves:,}")
    print(f"Initial Solution:        {initial_solution_source}")
    print(f"Initial SA Objective:    {initial_eval['total_cost']:.2f}")
    print(f"Initial SA Penalty:      {initial_eval['penalty']:.2f}")
    print(f"Initial Temperature:     {initial_temperature:.2f}")
    print(f"Final Temperature:       {temperature:.4f}")
    print(f"Operational Cost:        {detailed_eval['operational_cost']:.2f}")
    print(f"Penalty Cost:            {detailed_eval['penalty']:.2f}")
    print(f"SA Objective:            {detailed_eval['total_cost']:.2f}")

    report_lines, schedules = [], {}
    variable_report_str = ""
    solution_data = {}

    feasible = detailed_eval["penalty"] <= 1e-6

    if feasible:
        report_lines.append(
            f"Best SA solution found with total cost: {detailed_eval['operational_cost']:.2f}"
        )
    else:
        report_lines.append(
            f"Best SA solution found with penalized total cost: {detailed_eval['total_cost']:.2f}"
        )
        report_lines.append(
            f"  - Operating cost: {detailed_eval['operational_cost']:.2f}"
        )
        report_lines.append(
            f"  - Feasibility penalty: {detailed_eval['penalty']:.2f}"
        )

    used_vehicles = {
        vehicle for vehicle in vehicles
        if route_has_trip(best_state.get(vehicle, []))
    }
    report_lines.append(f"Total vehicles used: {len(used_vehicles)} out of {len(vehicles)}")

    for vehicle in sorted(list(used_vehicles)):
        visits = detailed_eval["visit_details"][vehicle]
        route = [item["node"] for item in visits]
        schedules[vehicle] = route
        ...
        for item in visits:
            ...
            if node_id.startswith("O"):
                energy_info = f"E_bar: {item['E_bar']:.1f}"
            else:
                energy_info = (
                    f"E_pre: {item['E_pre']:.1f}, "
                    f"G: {item['G']:.1f}, "
                    f"E_bar: {item['E_bar']:.1f}"
                )

            route_str.append(f"{node_id} (Time: {time_val:.2f}, {energy_info})")

        report_lines.append(f" - Vehicle {vehicle} schedule: {' -> '.join(route_str)}")

    var_report_lines = []

    # 1. Active arcs
    var_report_lines.append("\n--- Active Arc Variables (x[k,i,j] = 1) ---")
    sorted_x_keys = sorted(detailed_eval["active_arcs"].keys())
    for (vehicle, i, j) in sorted_x_keys:
        _, _, cons, cost = leg_metrics(vehicle, i, j)
        var_report_lines.append(
            f"  x[{vehicle}, {i}, {j}] = 1 "
            f"(Cost: {cost:.2f}, Cons: {cons:.1f})"
        )

    # 2. Node start times
    var_report_lines.append("\n--- Node Start Times (T[k,i] > 0) ---")
    sorted_T_keys = sorted(detailed_eval["node_times"].keys())
    for (vehicle, node_id) in sorted_T_keys:
        time_val = detailed_eval["node_times"][(vehicle, node_id)]
        if time_val > 0.0001:
            var_report_lines.append(f"  T[{vehicle}, {node_id}] = {time_val:.2f}")

    # 3. Waiting costs
    var_report_lines.append("\n--- Incurred Waiting Costs (w[k,i,j] > 0) ---")
    sorted_w_keys = sorted(detailed_eval["waiting_values"].keys())
    for (vehicle, i, j) in sorted_w_keys:
        wait_val = detailed_eval["waiting_values"][(vehicle, i, j)]
        if wait_val > 0.0001:
            var_report_lines.append(f"  w[{vehicle}, {i}, {j}] = {wait_val:.2f}")

    # 4. Energy variables
    var_report_lines.append("\n--- Energy Levels (E_pre, G, E_bar) ---")
    sorted_E_keys = sorted(detailed_eval["E_bar"].keys())
    for (vehicle, node_id) in sorted_E_keys:
        e_pre_val = detailed_eval["E_pre"].get((vehicle, node_id), 0.0)
        g_val = detailed_eval["G"].get((vehicle, node_id), 0.0)
        e_bar_val = detailed_eval["E_bar"].get((vehicle, node_id), 0.0)
        if e_bar_val > 0.0001:
            var_report_lines.append(
                f"  {vehicle}, {node_id}: E_pre={e_pre_val:.1f} | G={g_val:.1f} | "
                f"E_bar={e_bar_val:.1f} (Min/Max: {PHI_MIN}/{PHI_MAX})"
            )

    # 5. Charging completion times
    var_report_lines.append("\n--- Charging Completion Times (CT[k, j] > 0) ---")
    sorted_CT_keys = sorted(detailed_eval["CT"].keys())
    for (vehicle, node_id) in sorted_CT_keys:
        ct_val = detailed_eval["CT"][(vehicle, node_id)]
        if ct_val > 0.0001:
            var_report_lines.append(f"  CT[{vehicle}, {node_id}] = {ct_val:.2f}")

    # 6. SA diagnostics
    var_report_lines.append("\n--- SA Diagnostics ---")
    var_report_lines.append(f"  Initial solution: {initial_solution_source}")
    var_report_lines.append(f"  Initial SA objective: {initial_eval['total_cost']:.2f}")
    var_report_lines.append(f"  Initial SA penalty: {initial_eval['penalty']:.2f}")
    var_report_lines.append(f"  Rejected infeasible moves: {rejected_infeasible_moves}")
    var_report_lines.append(f"  Feasible: {feasible}")
    var_report_lines.append(f"  Operational cost: {detailed_eval['operational_cost']:.2f}")
    var_report_lines.append(f"  Penalty cost: {detailed_eval['penalty']:.2f}")
    var_report_lines.append(f"  Penalized objective: {detailed_eval['total_cost']:.2f}")

    variable_report_str = "\n".join(var_report_lines)

    # New dictionary to collect variable results for JSON export.
    solution_vars_json = {}
    solution_vars_json["x"] = {
        str(key).replace("'", "").replace(" ", ""): 1.0
        for key in detailed_eval["active_arcs"].keys()
    }
    solution_vars_json["T"] = {
        str(key).replace("'", "").replace(" ", ""): round(value, 4)
        for key, value in detailed_eval["node_times"].items()
        if value > 0.0001
    }
    solution_vars_json["w"] = {
        str(key).replace("'", "").replace(" ", ""): round(value, 4)
        for key, value in detailed_eval["waiting_values"].items()
        if value > 0.0001
    }
    solution_vars_json["E_pre"] = {
        str(key).replace("'", "").replace(" ", ""): round(value, 4)
        for key, value in detailed_eval["E_pre"].items()
        if value > 0.0001
    }
    solution_vars_json["E_bar"] = {
        str(key).replace("'", "").replace(" ", ""): round(value, 4)
        for key, value in detailed_eval["E_bar"].items()
        if value > 0.0001
    }
    solution_vars_json["G"] = {
        str(key).replace("'", "").replace(" ", ""): round(value, 4)
        for key, value in detailed_eval["G"].items()
        if abs(value) > 0.0001
    }
    solution_vars_json["CT"] = {
        str(key).replace("'", "").replace(" ", ""): round(value, 4)
        for key, value in detailed_eval["CT"].items()
        if value > 0.0001
    }
    solution_vars_json["tau"] = {
        str(key).replace("'", "").replace(" ", ""): round(value, 4)
        for key, value in detailed_eval["tau"].items()
        if value > 0.0001
    }

    solution_data["solution_vars"] = solution_vars_json

    # Preserve the original script style: write solution variables inside the
    # output directory created by the main execution block.
    if solution_data and output_dir is not None:
        json_output_path = os.path.join(output_dir, "sa_solution_variables.json")
        with open(json_output_path, "w") as f:
            json.dump(solution_data["solution_vars"], f, indent=4)
        print(f"SA solution variables saved to '{json_output_path}'")

    # Preserve the forward-state output used by the synthetic-data workflow.
    bus_end_states = {}
    if bus_end_states_dir is not None and run_timestamp is not None:
        for vehicle in vehicles:
            visits = detailed_eval["visit_details"].get(vehicle, [])
            if not visits or not route_has_trip(best_state.get(vehicle, [])):
                continue

            final_visit = visits[-1]
            final_node = final_visit["node"]
            if not final_node.startswith("D"):
                continue

            bus_end_states[vehicle] = {
                "buses_state": 0,
                "depot_id": int(final_node[1:]),
                "arrival_time": round(final_visit["time"], 4),
                "soc": round(final_visit["E_pre"], 4),
            }

        os.makedirs(bus_end_states_dir, exist_ok=True)
        bus_end_states_path = os.path.join(
            bus_end_states_dir,
            f"sa_bus_end_states_{run_timestamp}.json"
        )
        with open(bus_end_states_path, "w") as f:
            json.dump(bus_end_states, f, indent=4)
        print(f"SA bus end states saved to '{bus_end_states_path}'")

    stats = {
        "Iterations": max_iterations,
        "Accepted Moves": accepted_moves,
        "Improving Moves": improved_moves,
        "Rejected Infeasible": rejected_infeasible_moves,
        "Initial Objective": initial_eval["total_cost"],
        "Initial Penalty": initial_eval["penalty"],
        "SA Objective": detailed_eval["total_cost"],
        "Operational Cost": detailed_eval["operational_cost"],
        "Penalty Cost": detailed_eval["penalty"],
    }

    return "\n".join(report_lines), schedules, variable_report_str, stats, bus_end_states, None, detailed_eval["operational_cost"]
...
def solve_md_vsp_tw_from_instance(
    ...
    run_timestamp: str = None,
    mip_gap: float = None,
    time_limit: float = 18000,
    model_output_flag: int = 1
):
    """
    ...
    model.setParam('OutputFlag', model_output_flag)
    ...
    for k in vehicles:
        ...
        for trip_id, trip in trips_map.items():
            ...
            if t_oi <= u_i:
                ...
                energy_consumption[arc] = dist_oi * THETA_FACTOR
        ...
        for trip1_id, trip1 in trips_map.items():
            ...
            for trip2_id, trip2 in trips_map.items():
                ...
                if l_i + dur_i + t_ij <= u_j:
                    ...
                    energy_consumption[arc] = dist_ij * THETA_FACTOR
        ...
        for trip_id, trip in trips_map.items():
            ...
            dist_id = distance.euclidean(
                [trip.end_point.y, trip.end_point.x],
                [depot.location.y, depot.location.x]
            )
            t_id = positive_distance(dist_id)

            arc = (k, trip_id, d_node)
            arcs[arc] = 1
            base_costs[arc] = instance.meta["travel_cost"] * t_id
            elapsed_times[arc] = t_id
            energy_consumption[arc] = dist_id * THETA_FACTOR
        ...
        for cs_id, cs_obj in charging_station_map.items():
            ...
            for trip_id, trip in trips_map.items():
                ...
                dist_cj = distance.euclidean(
                    [cs_obj.location.y, cs_obj.location.x],
                    [trip.start_point.y, trip.start_point.x]
                )
                t_cj = positive_distance(dist_cj)

                arc = (k, cs_id, trip_id)
                arcs[arc] = 1
                base_costs[arc] = instance.meta["travel_cost"] * t_cj
                elapsed_times[arc] = t_cj
                energy_consumption[arc] = dist_cj * THETA_FACTOR
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        instance_path = os.path.join(project_root, "..", "input", "test_instances", "D2_S3_C20_d_trips.txt")
    ...
    solution_report, schedules, variable_report_str, stats, _, _, _ = solve_md_vsp_tw_sa_from_instance(
        instance,
        output_dir=output_dir,
        bus_end_states_dir=bus_end_states_output_dir,
        run_timestamp=timestamp,
        use_gurobi_initial_solution=True,
        gurobi_initial_gap=0.2,
        gurobi_initial_output_flag=1,
    ...
    with open(full_report_path, "w", encoding='utf-8') as f:
        ...
        f.write("\n\n--- SOLUTION ---\n")
        if isinstance(solution_report, list):
