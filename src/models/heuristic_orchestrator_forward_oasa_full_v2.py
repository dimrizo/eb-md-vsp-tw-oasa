...
"""
Stateful rolling-horizon orchestrator for the full OASA dataset with selective exact cluster merging.
...
SELECTIVE_MERGE_MAX_TRIPS_PER_WINDOW = 10  # same selective-merge cap as oasa_v2; raise if full clusters are larger
...
def write_merged_trips_file(blocks: List[ScheduleBlock], output_dir: str, label: str) -> str:
    ...
    seen = set()
    merged_trip_ids: List[str] = []

    for block in blocks:
        for trip_id in read_trip_ids_from_txt(block.trips_txt):
            if trip_id not in seen:
                seen.add(trip_id)
                merged_trip_ids.append(trip_id)

    with open(merged_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["trip_id"])
        for trip_id in merged_trip_ids:
            writer.writerow([trip_id])
