...
instance_path = os.path.join(project_root, "..", "input", "test_instances", "D2_S3_C20_a_trips.txt")
instance_charge_seq_path = os.path.join(project_root, "..", "input", "test_instances", "D2_S3_C20_charging_event_sequence.txt")


data_header_charg_event=np.loadtxt(instance_charge_seq_path, max_rows=1,dtype=int)
data_main_body_charg_event=np.loadtxt(instance_charge_seq_path, skiprows=1,dtype=int)
...
data_header=np.loadtxt(instance_path, max_rows=1, dtype=int)
data_header_decimal=np.loadtxt(instance_path, max_rows=1, dtype=float)
data_main_body=np.loadtxt(instance_path, skiprows=1, dtype=int)
...
for i in V:
    ...
    t_tilde[i] = distance.euclidean([latitude_of_node_i_start,longitude_of_node_i_start],[latitude_of_node_i_end,longitude_of_node_i_end])
    eta[i] = e_consumption*distance.euclidean([latitude_of_node_i_start,longitude_of_node_i_start],[latitude_of_node_i_end,longitude_of_node_i_end])
...
A={}
for k in K:
    A_a=[(O[k],j) for j in Nk[k] if j!=O[k]] #arcs_from_origin_depot
    A_b=[(i,D[k]) for i in Nk[k] if i not in [O[k],D[k]]] #arcs_to_destination_depot
    A_c=[(i,j) for i in Vk[k] for j in Vk[k] if i!=j] #arcs_from_trip_to_trip
    A_d=[(i,j) for i in Vk[k] for j in F] #arcs_from_trip_to_charging event
    A_f=[(i,j) for i in F for j in Vk[k]] #arcs_from_chargingevent_to_trip
...
M=100000 #very large positive number
...
#VALID INEQUALITIES
#model.addConstrs(e[j,k]<=phi_max[k] for k in K for j in Vk[k]+F+[D[k]]) #eq.(51)
#model.addConstrs(e_bar[i,k]>=phi_min[k] + float(theta[i,q[i]]) for k in K for i in Vk[k]) #eq.(52)
#model.addConstrs(sum(y[i,k]*l[i] for k in K) <= T[omega[i],k] + M*(1- sum( x[l,r,k] for l,r in A[k] if l==omega[i] ) ) for i in F_no_end for k in K) #eq.(53)
#model.addConstrs(T[i,k]+t_tilde[i]+t[i,j]<=u[j] for k in K for i,j in A[k] if i in Vk[k]) #eq.(54)
#model.addConstrs(T[i,k]+tau[i,k]+t[i,j]<=u[j] for k in K for i,j in A[k] if i in F) #eq.(55)
